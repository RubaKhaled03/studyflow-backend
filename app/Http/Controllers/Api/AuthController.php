...
public function updateProfile(Request $request)
{
    ...
    public function forgotPassword(Request $request)
    {
        ...
        // With MAIL_MAILER=log (the project default) this is written to
        // storage/logs/laravel.log instead of actually being emailed —
        // good enough for development/demo without needing real SMTP credentials.
        Mail::raw("Reset your StudyFlow password using this link: {$resetLink}", function ($message) use ($user) {
            $message->to($user->email)->subject('Reset your StudyFlow password');
        });

        return response()->json([
            'message' => 'Reset link sent successfully.',
            // Returned directly so the frontend can skip waiting on real email
            // delivery during development. Remove this in a production deployment.
            'token'   => $token,
            'email'   => $user->email,
        ]);
    }
